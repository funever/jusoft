...
public class ColorPickerDialog extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(manager.getDefaultDisplay().getHeight()<manager.getDefaultDisplay().getWidth()){
        	height =  (int) (manager.getDefaultDisplay().getHeight() * 0.75f);
		...
		colorText = new TextView(context);
		colorText.setTextColor(Color.WHITE);
		colorText.setText("#FFFFFF");
		colorText.setTextSize(16);
		colorText.setGravity(Gravity.CENTER);
		layout.addView(colorText, new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
		ColorPickerView myView = new ColorPickerView(context, height, width);
		layout.addView(myView, new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
                ...
                case MotionEvent.ACTION_MOVE:
                	if(downInCircle && inCircle) {//down按在渐变色环内, 且move也在渐变色环内
                        ...
                        if (unit < 0) {
	               		...
	               		colorText.setText("0x"+Integer.toHexString(interpRectColor(mRectColors, x)));
